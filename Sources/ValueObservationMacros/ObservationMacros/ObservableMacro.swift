...
public struct ObservableValueMacro {
    static let moduleName = "ValueObservation"

    static let conformanceName = "ObservableValue"
    ...
    static let trackedMacroName = "Observing"
    static let ignoredMacroName = "Ignoring"
...
extension ObservableValueMacro: MemberMacro {
    ...
    >(
        ...
        if declaration.isActor {
            ...
            throw DiagnosticsError(syntax: node, message: "'@ObservableValue' cannot be applied to actor type '\(observableType.text)'", id: .invalidApplication)
        }

        var declarations = [DeclSyntax]()

        let accessLevelKeyword = declaration.accessLevelKeyword
        declaration.addIfNeeded(
            ObservableValueMacro.identifierVariable(observableType, accessLevelKeyword: accessLevelKeyword),
            to: &declarations
        )
        declaration.addIfNeeded(ObservableValueMacro.registrarVariable(observableType, context: context), to: &declarations)
        declaration.addIfNeeded(ObservableValueMacro.copyFunction(observableType, accessLevelKeyword: accessLevelKeyword), to: &declarations)
        declaration.addIfNeeded(ObservableValueMacro.accessFunction(observableType, context: context), to: &declarations)
        declaration.addIfNeeded(ObservableValueMacro.withMutationFunction(observableType, context: context), to: &declarations)
        declaration.addIfNeeded(ObservableValueMacro.shouldNotifyObserversNonEquatableFunction(observableType, context: context), to: &declarations)
        declaration.addIfNeeded(ObservableValueMacro.shouldNotifyObserversEquatableFunction(observableType, context: context), to: &declarations)
        declaration.addIfNeeded(ObservableValueMacro.shouldNotifyObserversNonEquatableObjectFunction(observableType, context: context), to: &declarations)
        declaration.addIfNeeded(ObservableValueMacro.shouldNotifyObserversEquatableObjectFunction(observableType, context: context), to: &declarations)

        return declarations
    }
}

extension ObservableValueMacro: MemberAttributeMacro {
    public static func expansion<
        Declaration: DeclGroupSyntax,
    ...
    >(
        ...
        if property.hasMacroApplication(ObservableValueMacro.ignoredMacroName) ||
            property.hasMacroApplication(ObservableValueMacro.trackedMacroName) {
            return []
        }


        return [
            AttributeSyntax(attributeName: IdentifierTypeSyntax(name: .identifier(ObservableValueMacro.trackedMacroName)))
        ]
    }
}

extension ObservableValueMacro: ExtensionMacro {
    public static func expansion(
        of node: AttributeSyntax,
...
public struct ObservingMacro: AccessorMacro {
    public static func expansion<
        Context: MacroExpansionContext,
    ...
    >(
        ...
        guard context.lexicalContext.contains(where: { decl in
            decl.is(ClassDeclSyntax.self) || decl.is(StructDeclSyntax.self)
        }) else {
            return []
        }
      ...
      """
        ...
        _modify {
          ...
          if _\(identifier) is ObservableValue {
            ...
            defer { \(raw: ObservableValueMacro.registrarVariableName).didSet(self, keyPath: \\.\(identifier)) }
            yield &_\(identifier)
          }
        }
        """
...
extension ObservingMacro: PeerMacro {
    public static func expansion<
        Context: MacroExpansionContext,
        Declaration: DeclSyntaxProtocol
    >(
        ...
        let storage = DeclSyntax(property.privatePrefixed("_", addingAttribute: ObservableValueMacro.ignoredAttribute, removingAttribute: ObservableValueMacro.trackedAttribute, in: localContext))
        return [storage]
    }
}

public struct IgnoringMacro: PeerMacro {
    public static func expansion<
        Context: MacroExpansionContext,
        Declaration: DeclSyntaxProtocol
    >(
